...
if u1 > 0.01&&u2>0.01    %%u1>0 and u1 !=1  and u2>0
...
elseif u1<-0.01&&u2<-0.01 %% u1<0 and u1!=-1 and u2<0
...
elseif u1<-0.01&&u2>0.01 %% u1<0 and u1 !=-1 and u2>0
    ...
    Zh=u1;
    Yh=u2;
    t1=(cross(z,v)*Zh*OZ-abs(Zh)*cross(G,v))*AZES;%%torque_due_to_the_deployable_solar_plan.
    y=[0,1,0];
