...
n = length(PID_angle_error_rate_x(1,:)); %number of simulations
t_sim = length(PID_angle_error_rate_x(:,1)); %duration of the simulations
...
for i = 1:n
    ...
    for j=t_steady_state:t_sim
        if (mean(PID_angle_error_rate_x(j,i))>5) || (mean(PID_angle_error_rate_x(j,i))<-5)
            C1_failed_x = C1_failed_x+1;
        end
        if (mean(PID_angle_error_rate_y(j,i))>5) || (mean(PID_angle_error_rate_y(j,i))<-5)
            C1_failed_y = C1_failed_y+1;
        end
        if (mean(PID_angle_error_rate_z(j,i))>5) || (mean(PID_angle_error_rate_z(j,i))<-5)
...
%% Criteria 1bis : on the angle error rate, in caseboth PID and PID fails C1 at 100%
...
for i = 1:n
    ...
    %if during the last 1000 seconds of simulation the criteria isn't validated, then it failed
    for j=t_steady_state:t_sim
        if (mean(PID_angle_error_rate_x(j,i))>15) || (mean(PID_angle_error_rate_x(j,i))<-15)
            C1bis_failed_x = C1bis_failed_x+1;
        end
        if (mean(PID_angle_error_rate_y(j,i))>15) || (mean(PID_angle_error_rate_y(j,i))<-15)
            C1bis_failed_y = C1bis_failed_y+1;
        end
        if (mean(PID_angle_error_rate_z(j,i))>15) || (mean(PID_angle_error_rate_z(j,i))<-15)
...
for i = 1:n
    ...
    for j=t_steady_state:t_sim
        if (mean(PID_angular_velocity_error_rate_x(j,i))>0.5) || (mean(PID_angular_velocity_error_rate_x(j,i))<-0.5)
            C2_failed_x = C2_failed_x+1;
        end
        if (mean(PID_angular_velocity_error_rate_y(j,i))>0.5) || (mean(PID_angular_velocity_error_rate_y(j,i))<-0.5)
            C2_failed_y = C2_failed_y+1;
        end
        if (mean(PID_angular_velocity_error_rate_z(j,i))>0.5) || (mean(PID_angular_velocity_error_rate_z(j,i))<-0.5)
...
%% Criteria 2bis : on the angular velocity error rate, in case both PID and PID fails C2 at 100%
...
for i = 1:n
    ...
    %if during the last 1000 seconds of simulation the criteria isn't validated, then it failed
    for j=t_steady_state:t_sim
        if (mean(PID_angular_velocity_error_rate_x(j,i))>1.5) || (mean(PID_angular_velocity_error_rate_x(j,i))<-1.5)
            C2bis_failed_x = C2bis_failed_x+1;
        end
        if (mean(PID_angular_velocity_error_rate_y(j,i))>1.5) || (mean(PID_angular_velocity_error_rate_y(j,i))<-1.5)
            C2bis_failed_y = C2bis_failed_y+1;
        end
        if (mean(PID_angular_velocity_error_rate_z(j,i))>1.5) || (mean(PID_angular_velocity_error_rate_z(j,i))<-1.5)
...
%loop that increments the previous variables
for i=1:n
    if PID_RW_average_power_consumtion(i) > 3
...
%loop that increments the previous variables
for i=1:n
    average_saturation_all_RW = mean(PID_RW_saturation_duration(:,i)); %compute the average saturation duration of all the RW
...
%% Criteria 4bis : on the saturation of the RW; in case both PID and PID fails C4 at 100%
...
n=length(Attitude_real_vs_estimated_x(1,:));
...
Criteria = categorical({'C1','C1bis','C2','C2bis','C3','C4','C4bis','C5','C5bis'});
Criteria = reordercats(Criteria,{'C1','C1bis','C2','C2bis','C3','C4','C4bis','C5','C5bis'});
Failed_percentage = [Failed_percentage_average_C1;Failed_percentage_average_C1bis;Failed_percentage_average_C2;Failed_percentage_average_C2bis;Failed_percentage_C3;Failed_percentage_C4;Failed_percentage_C4bis;Failed_percentage_average_C5;Failed_percentage_average_C5bis];
figure;
...
PID_failed_average_mangitude_angle_estimation_error = zeros(3,n_failed_C5);
PID_failed_RW_average_power_consumtion =zeros(1,n_failed_C2);

for i=1:n_failed_C1
    failed_index = Failed_index_list_C1(i);
    PID_failed_average_magnitude_angle_error(:,i) = PID_average_magnitude_angle_error(failed_index,:);
end

for i=1:n_failed_C2
    failed_index = Failed_index_list_C2(i);
    PID_failed_average_magnitude_angular_velocities_error(:,i) = PID_average_magnitude_angular_velocities_error(failed_index,:);
    PID_failed_RW_average_power_consumtion(i) = PID_RW_average_power_consumtion(failed_index);

end

for i=1:n_failed_C5
    failed_index = Failed_index_list_C5(i);
    PID_failed_average_mangitude_angle_estimation_error(:,i) = Attitude_average_mangnitude_real_vs_estimated(failed_index,:);
end
...
hold on;
scatter(Failed_index_list_C1,PID_failed_average_magnitude_angle_error(1,:));
scatter(Failed_index_list_C1,PID_failed_average_magnitude_angle_error(2,:));
scatter(Failed_index_list_C1,PID_failed_average_magnitude_angle_error(3,:));
...
nexttile;
hold on;
scatter(Failed_index_list_C2,PID_failed_average_magnitude_angular_velocities_error(1,:));
scatter(Failed_index_list_C2,PID_failed_average_magnitude_angular_velocities_error(2,:));
scatter(Failed_index_list_C2,PID_failed_average_magnitude_angular_velocities_error(3,:));
legend('X axis','Y axis','Z axis')
xlabel('n-th simulation');
...
nexttile;
hold on;
scatter(Failed_index_list_C5,PID_failed_average_mangitude_angle_estimation_error(1,:));
scatter(Failed_index_list_C5,PID_failed_average_mangitude_angle_estimation_error(2,:));
scatter(Failed_index_list_C5,PID_failed_average_mangitude_angle_estimation_error(3,:));
legend('X axis','Y axis','Z axis')
xlabel('n-th simulation');
ylabel('Angular velocities error (in °/s)');
...
figure;
hold on;
scatter(Failed_index_list_C2,PID_failed_RW_average_power_consumtion);
xlabel('n-th simulation');
