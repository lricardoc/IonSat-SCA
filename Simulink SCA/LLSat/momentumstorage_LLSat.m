...
d=pi/180;           %deviation in rads of the thruster wrt to x axis, this is the initial in plane xy
Time= 3;      %time of thrust in seconds (3 sec)

%the deviation is of d=1 degree
F=0.5*[cos(d);sin(d);0];  %F in [N] 
x_cog=0;        %Distance of CoG from the geometric center x axis in mm
% y_cog=3.81;     %Distance of CoG from the geometric center y axis in mm
% z_cog=8.23;     %Distance of CoG from the geometric center z axis in mm
x_th=150;       %Position x of point of thrust
y_th=0;         %Position y of point of thrust
z_th=0;         %Position z of point of thrust
...
r=1.;            %radius of circle around YZ plane where the CoG lies
...
d=0.1*pi/180;           %deviation in rads of the thruster wrt to x axis, this is the initial in plane xy
Time= 3;      %time of thrust in seconds (50 minutes)
...
a=0;F_rot=[0;0;0];
%MMS=[0;0;0];      %initialize variables
r=1.2;            %radius of circle around YZ plane where the CoG lies
...
% max (MMSz,[],'all')
% max (MMSnorm,[],'all')
%we know that MMS for each wheel is 2 mNms,for RWA is 
% =4 mNms in each axis 
%taking a 10% margin
%this is the maximum distance of the CoG from the Geometric Center. in Y-Z


%% TORQUE SAME AS BEFORE BUT JUST TO PLOT XI  ξ and ψ 
clear
...
for j=1:360/step_angle+1
    ...
    for i=1:360/step_angle+1    %rotate F around x axis
        ...
        F_rot(:,i)=R*F;
        %MMS(i,j,:)=cross(F_rot(:,i),D)*1e3*Time;
        Tor_vec=cross(F_rot(:,i),D)*1e3;   %Torque in mNm
        Tor_norm=norm(Tor_vec);
        MTor(i,j,1:3)=Tor_vec;
        MTor(i,j,4)=Tor_norm;
    end
end
...
set(gcf,'color','black');
    subplot(2,2,1)
        ...
        xlabel('rotation alpha dev. F around X starting in XY')
        grid on
        title('Required Torque in X axis [mNm]') 
        colorbar
    subplot(2,2,2)
        mesh(a,b,MTory);
        ylabel('rotation beta CoG in XY')
        xlabel('rotation alpha dev. F around X starting in XY')
        grid on
        title('Required Torque in Y axis [mNm]')   
        colorbar
    subplot(2,2,3)
        mesh(a,b,MTorz);
        ylabel('rotation beta CoG in XY')
        xlabel('rotation alpha dev. F around X starting in XY')
        grid on
        title('Required Torque in Z axis [mNm]')   
        colorbar
    subplot(2,2,4)
        mesh(a,b,MTornorm);
        ylabel('rotation beta CoG in XY')
        xlabel('rotation alpha dev. F around X starting in XY')
        grid on
        title('Required Torque norm [mNm]') 
        colorbar
        
% max (MMSy,[],'all')        
% max (MMSz,[],'all')
% max (MMSnorm,[],'all')
%we know that MMS for each wheel is 0.2 mNms,for RWA is 
% =0.4 mNms in each axis 
%r is the maximum distance of the CoG from the Geometric Center. in Y-Z
