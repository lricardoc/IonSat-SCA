...
MEKF_angle_error_rate_x = zeros(n_points,n);
MEKF_angle_error_rate_y = zeros(n_points,n);
MEKF_angle_error_rate_z = zeros(n_points,n);

MEKF_angular_velocity_error_rate_x = zeros(n_points,n);
MEKF_angular_velocity_error_rate_y = zeros(n_points,n);
MEKF_angular_velocity_error_rate_z = zeros(n_points,n);

MEKF_angle_estimation_error_x = zeros(n_points,n);
MEKF_angle_estimation_error_y = zeros(n_points,n);
MEKF_angle_estimation_error_z = zeros(n_points,n);

MEKF_bias_x = zeros(n_points,n);
MEKF_bias_y = zeros(n_points,n);
MEKF_bias_z = zeros(n_points,n);
...
%     sat.sensors.gyro_bias=0.160e-05; %to test
    ...
    % run the simulink 
    simResults = sim('MEKFsim_v2.slx');
    
    %save the data
    MEKF_angle_error_rate_x(:,i) = simResults.angle_error_rate.Data(:,1);
    MEKF_angle_error_rate_y(:,i) = simResults.angle_error_rate.Data(:,2);
    MEKF_angle_error_rate_z(:,i) = simResults.angle_error_rate.Data(:,3);

    MEKF_angular_velocity_error_rate_x(:,i) = simResults.angular_velocities_error_rate.Data(:,1);
    MEKF_angular_velocity_error_rate_y(:,i) = simResults.angular_velocities_error_rate.Data(:,2);
    MEKF_angular_velocity_error_rate_z(:,i) = simResults.angular_velocities_error_rate.Data(:,3);
    
    MEKF_angle_estimation_error_x(:,i) = simResults.angle_estimation_error.Data(1,:);
    MEKF_angle_estimation_error_y(:,i) = simResults.angle_estimation_error.Data(2,:);
    MEKF_angle_estimation_error_z(:,i) = simResults.angle_estimation_error.Data(3,:);
...
%Data processing
MEKF_average_magnitude_angle_error = zeros(n,3);

MEKF_average_magnitude_angular_velocities_error = zeros(n,3);

MEKF_average_magnitude_angle_estimation_error = zeros(n,3);

MEKF_average_magnitude_bias = zeros(n,3);


for k =1:n
    %Compute the average magnitude (RMS) of the angle error
    MEKF_average_magnitude_angle_error(k,1) = sqrt(mean(MEKF_angle_error_rate_x(:,k).^2 ));
    MEKF_average_magnitude_angle_error(k,2) = sqrt(mean(MEKF_angle_error_rate_y(:,k).^2 ));
    MEKF_average_magnitude_angle_error(k,3) = sqrt(mean(MEKF_angle_error_rate_z(:,k).^2 ));

    %Compute the average magnitude (RMS) of the angular velocities error
    MEKF_average_magnitude_angular_velocities_error(k,1) = sqrt(mean(MEKF_angular_velocity_error_rate_x(:,k).^2 ));
    MEKF_average_magnitude_angular_velocities_error(k,2) = sqrt(mean(MEKF_angular_velocity_error_rate_y(:,k).^2 ));
    MEKF_average_magnitude_angular_velocities_error(k,3) = sqrt(mean(MEKF_angular_velocity_error_rate_z(:,k).^2 ));
    
    %Compute the average magnitude (RMS) of the angle estimation error
    MEKF_average_magnitude_angle_estimation_error(k,1) = sqrt(mean(MEKF_angle_estimation_error_x(:,k).^2 ));
    MEKF_average_magnitude_angle_estimation_error(k,2) = sqrt(mean(MEKF_angle_estimation_error_y(:,k).^2 ));
    MEKF_average_magnitude_angle_estimation_error(k,3) = sqrt(mean(MEKF_angle_estimation_error_z(:,k).^2 ));
    
     %Compute the average magnitude (RMS) of the angle estimation error
    MEKF_average_magnitude_bias(k,1) = sqrt(mean(MEKF_bias_x(:,k).^2 ));
    MEKF_average_magnitude_bias(k,2) = sqrt(mean(MEKF_bias_y(:,k).^2 ));
