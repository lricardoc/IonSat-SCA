...
n = length(UKF_angle_error_rate_x(1,:)); %number of simulations
t_sim = length(UKF_angle_error_rate_x(:,1)); %duration of the simulations
...
for i = 1:n
    ...
    for j=t_steady_state:t_sim
        if (mean(UKF_angle_error_rate_x(j,i))>5) || (mean(UKF_angle_error_rate_x(j,i))<-5)
            C1_failed_x = C1_failed_x+1;
        end
        if (mean(UKF_angle_error_rate_y(j,i))>5) || (mean(UKF_angle_error_rate_y(j,i))<-5)
            C1_failed_y = C1_failed_y+1;
        end
        if (mean(UKF_angle_error_rate_z(j,i))>5) || (mean(UKF_angle_error_rate_z(j,i))<-5)
...
%% Criteria 1bis : on the angle error rate, in caseboth UKF and UKF fails C1 at 100%
...
for i = 1:n
    ...
    %if during the last 1000 seconds of simulation the criteria isn't validated, then it failed
    for j=t_steady_state:t_sim
        if (mean(UKF_angle_error_rate_x(j,i))>15) || (mean(UKF_angle_error_rate_x(j,i))<-15)
            C1bis_failed_x = C1bis_failed_x+1;
        end
        if (mean(UKF_angle_error_rate_y(j,i))>15) || (mean(UKF_angle_error_rate_y(j,i))<-15)
            C1bis_failed_y = C1bis_failed_y+1;
        end
        if (mean(UKF_angle_error_rate_z(j,i))>15) || (mean(UKF_angle_error_rate_z(j,i))<-15)
...
for i = 1:n
    ...
    for j=t_steady_state:t_sim
        if (mean(UKF_angular_velocity_error_rate_x(j,i))>0.5) || (mean(UKF_angular_velocity_error_rate_x(j,i))<-0.5)
            C2_failed_x = C2_failed_x+1;
        end
        if (mean(UKF_angular_velocity_error_rate_y(j,i))>0.5) || (mean(UKF_angular_velocity_error_rate_y(j,i))<-0.5)
            C2_failed_y = C2_failed_y+1;
        end
        if (mean(UKF_angular_velocity_error_rate_z(j,i))>0.5) || (mean(UKF_angular_velocity_error_rate_z(j,i))<-0.5)
...
%% Criteria 2bis : on the angular velocity error rate, in case both UKF and UKF fails C2 at 100%
...
for i = 1:n
    ...
    %if during the last 1000 seconds of simulation the criteria isn't validated, then it failed
    for j=t_steady_state:t_sim
        if (mean(UKF_angular_velocity_error_rate_x(j,i))>1.5) || (mean(UKF_angular_velocity_error_rate_x(j,i))<-1.5)
            C2bis_failed_x = C2bis_failed_x+1;
        end
        if (mean(UKF_angular_velocity_error_rate_y(j,i))>1.5) || (mean(UKF_angular_velocity_error_rate_y(j,i))<-1.5)
            C2bis_failed_y = C2bis_failed_y+1;
        end
        if (mean(UKF_angular_velocity_error_rate_z(j,i))>1.5) || (mean(UKF_angular_velocity_error_rate_z(j,i))<-1.5)
...
UKF_failed_average_magnitude_angle_error = zeros(3,n_failed_C1); 
UKF_failed_average_magnitude_angular_velocities_error = zeros(3,n_failed_C2); 
UKF_failed_average_mangitude_angle_estimation_error = zeros(3,n_failed_C5);

for i=1:n_failed_C1
    failed_index = Failed_index_list_C1(i);
    UKF_failed_average_magnitude_angle_error(:,i) = UKF_average_magnitude_angle_error(failed_index,:);
end

for i=1:n_failed_C2
    failed_index = Failed_index_list_C2(i);
    UKF_failed_average_magnitude_angular_velocities_error(:,i) = UKF_average_magnitude_angular_velocities_error(failed_index,:);
end

for i=1:n_failed_C5
    failed_index = Failed_index_list_C5(i);
    UKF_failed_average_mangitude_angle_estimation_error(:,i) = Attitude_average_mangnitude_real_vs_estimated(failed_index,:);
end
...
hold on;
scatter(Failed_index_list_C1,UKF_failed_average_magnitude_angle_error(1,:));
scatter(Failed_index_list_C1,UKF_failed_average_magnitude_angle_error(2,:));
scatter(Failed_index_list_C1,UKF_failed_average_magnitude_angle_error(3,:));
...
nexttile;
hold on;
scatter(Failed_index_list_C2,UKF_failed_average_magnitude_angular_velocities_error(1,:));
scatter(Failed_index_list_C2,UKF_failed_average_magnitude_angular_velocities_error(2,:));
scatter(Failed_index_list_C2,UKF_failed_average_magnitude_angular_velocities_error(3,:));
legend('X axis','Y axis','Z axis')
xlabel('n-th simulation');
...
nexttile;
hold on;
scatter(Failed_index_list_C5,UKF_failed_average_mangitude_angle_estimation_error(1,:));
scatter(Failed_index_list_C5,UKF_failed_average_mangitude_angle_estimation_error(2,:));
scatter(Failed_index_list_C5,UKF_failed_average_mangitude_angle_estimation_error(3,:));
legend('X axis','Y axis','Z axis')
xlabel('n-th simulation');
ylabel('Angular velocities error (in °/s)');
